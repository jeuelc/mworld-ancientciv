/* i-C-a */
$('.menubtn').on('tap', function() {
});


$('.cpbtnholder').on('tap', function() {
  $.mobile.changePage("controlpanel.html");
  mainaudiobg.play(); bgsoundEarlyChina.pause(); bgsoundEarlyChina.currentTime = 0; 
});

$('.menubtnEarlyChina').on('tap', function() {
  $(this).css('background-position', '0px 0px');
  setTimeout(function() {
      $('.menubtnEarlyChina').css('background-position', '0px -90px');
    }, 300);
  var housingpos = $('.menuhousing').position();
  if(housingpos.left == -234) {
    $('.menuhousing').animate({'left':'-334px','opacity':0}, 500, "easeInBack", function() { $('.menuhousing').css({'left':'-660px'}); });
    $('.homebtnholder').animate({'left':'3px','opacity':0}, 500, "easeInBack", function() { $('.homebtnholder').css({'left':'-93px'}); });
    $('.mpbtnholder').animate({'left':'80px','opacity':0}, 500, "easeInBack", function() { $('.mpbtnholder').css({'left':'-93px'}); });
    $('.cpbtnholder').animate({'left':'153px','opacity':0}, 500, "easeInBack", function() { $('.cpbtnholder').css({'left':'-93px'}); });
    $('.infobtnholder').animate({'left':'222px','opacity':0}, 500, "easeInBack", function() { $('.infobtnholder').css({'left':'-93px'}); });
    $('.credsbtnholder').animate({'left':'288px','opacity':0}, 500, "easeInBack", function() { $('.credsbtnholder').css({'left':'-93px'}); });
  } else {
    $('.menuhousing').css({'opacity':1});
    $('.credsbtnholder').css({'opacity':1});
    $('.infobtnholder').css({'opacity':1});
    $('.cpbtnholder').css({'opacity':1});
    $('.mpbtnholder').css({'opacity':1});
    $('.homebtnholder').css({'opacity':1});
    $('.menuhousing').animate({'left':'-234px'}, 1000, "easeInOutBack");
    $('.credsbtnholder').animate({'left':'368px'}, 1000, "easeInOutBack");
    $('.infobtnholder').animate({'left':'302px'}, 1100, "easeInOutBack");
    $('.cpbtnholder').animate({'left':'233px'}, 1200, "easeInOutBack");
    $('.mpbtnholder').animate({'left':'160px'}, 1300, "easeInOutBack");
    $('.homebtnholder').animate({'left':'83px'}, 1400, "easeInOutBack");
  }
});

$('.homebtnEarlyChina').on('tap', function() {
  $(this).css('background-position', '0px 0px');
  setTimeout(function() {
      $('.homebtnEarlyChina').css('background-position', '0px -74px');
    }, 300);
});

$('.mpbtnEarlyChina').on('tap', function() {
  $(this).css('background-position', '0px -3px');
  setTimeout(function() {
      $('.mpbtnEarlyChina').css('background-position', '0px -84px');
    }, 300);
});

$('.cpbtnEarlyChina').on('tap', function() {
  $(this).css('background-position', '0px 0px');
  setTimeout(function() {
      $('.cpbtnEarlyChina').css('background-position', '0px -74px');
    }, 300);
});

$('.infobtnEarlyChina').on('tap', function() {
  $(this).css('background-position', '0px 0px');
  setTimeout(function() {
      $('.infobtnEarlyChina').css('background-position', '0px -74px');
    }, 300);
});

$('.credsbtnEarlyChina').on('tap', function() {
  $(this).css('background-position', '0px 0px');
  setTimeout(function() {
      $('.credsbtnEarlyChina').css('background-position', '0px -74px');
    }, 300);
});

$('.micbtnEarlyChina').on('tap', function() {
  $(this).css('background-position-y', '0px');
  setTimeout(function() {
      $('.micbtnEarlyChina').css('background-position-y', '-81px');
    }, 300);
  var botsposition = $('.bEarlyChina').position();
  if (botsposition.left == 1024) {
    $('.bEarlyChina').css({'opacity':1});
    $('.plEarlyChina').css({'opacity':1});
    $('.fEarlyChina').css({'opacity':1});
    $('.bEarlyChina').animate({'left':'742px'}, 1000, "easeInOutBack");
    $('.plEarlyChina').animate({'left':'808px'}, 1100, "easeInOutBack");
    $('.fEarlyChina').animate({'left':'876px'}, 1200, "easeInOutBack");
  } else {
    $('.bEarlyChina').animate({'left':'842px','opacity':0}, 500, "easeInBack", function() { $('.bEarlyChina').css({'left':'1024px'}); });
    $('.plEarlyChina').animate({'left':'908px','opacity':0}, 500, "easeInBack", function() { $('.plEarlyChina').css({'left':'1024px'}); });
    $('.fEarlyChina').animate({'left':'976px','opacity':0}, 500, "easeInBack", function() { $('.fEarlyChina').css({'left':'1024px'}); });
  }
});

$('.rwEarlyChina').on('tap', function() {
  $(this).css('background-position-y', '0px');
  setTimeout(function() {
      $('.rwEarlyChina').css('background-position-y', '-73px');
    }, 300);
});

$('.plpsEarlyChina').on('tap', function() {
  $(this).css('background-position-y', '0px');
  setTimeout(function() {
      $('.plpsEarlyChina').css('background-position-y', '-73px');
    }, 300);
});

$('.fwEarlyChina').on('tap', function() {
  $(this).css('background-position-y', '0px');
  setTimeout(function() {
      $('.fwEarlyChina').css('background-position-y', '-73px');
    }, 300);
});

if(typeof window.lyrics_checked == 'undefined'){
	window.lyrics_checked		=	0;
}
if(typeof window.melody_checked == 'undefined'){
	window.melody_checked		=	0;
}
if(typeof window.beat_checked == 'undefined'){
	window.beat_checked		=	0;
}
if(typeof window.chords_checked == 'undefined'){
	window.chords_checked		=	0;
}